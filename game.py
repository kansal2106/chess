...
def initialize_piece():
    ...
    while i < len(pie):
        if pie[i].rank == "p" and pie[i].life:
            ...
            game_display.blit(img_small, ((2 * pie[i].x) * factor, ((2 * pie[i].y) * factor)))
...
def diagonal(i_x, i_y, f_x, f_y):
    ...
    while a != f_x and b != f_y:
        for i in range(len(pie)):
            # print("a =", a, ",b =", b)
            if pie[i].x == a + dir_x and pie[i].y == b + dir_y and pie[i].x!=f_x and pie[i].y!=f_y:
                th = False
...
def board_draw():
    ...
    for i in range(8):
        ...
        while j < 8:
            ...
            j += 2
    for i in range(9):
        pygame.draw.line(game_display, black, (0,i*75), (600,i*75), 2)
    for i in range(1,9):
        pygame.draw.line(game_display, black, (i*75,0), (i*75,600), 2)
...
def check_king_life():
    # print("inside king life function")
    global selected_family
    val = True
    for i in range(len(pie)):
        # print("inside for loop")
        if pie[i].rank == "king" and pie[i].life == False:
            # print("inside if block")
            myfont = pygame.font.Font("sans.ttf", 50)
            strin = "Congratulations"
            if selected_family == "black" :
                string = "White!!!"
            else:
                string = "Black!!!"
            label1 = myfont.render(strin, 1, red)
            label = myfont.render(string, 1, red)
            game_display.blit(label1, (130, 230))
            game_display.blit(label, (228, 310))
            pygame.display.update()
            time.sleep(1)
            val = False
    return val

def check_king_danger():
    global selected_family
    val = False
    b_k = 0 #black king
    w_k = 0 #white king
    for i in range(len(pie)):
        if pie[i].rank == "king":
            ...
            if pie[i].family == "white":
                w_k = i
    # print("w_k = ",pie[w_k].x,pie[w_k].y)
    # check for white king
    for i in range(len(pie)):
        #check with black pawn
        if pie[i].rank == 'p' and pie[i].family == "black":
            if pie[i].y == pie[w_k].y + 1 :
                if pie[i].x == pie[w_k].x+1 or pie[i].x == pie[w_k].x-1:
                    val = True

        #check with black rook
        if pie[i].rank == 'r' and pie[i].family == "black":
            if check_rook(pie[w_k].x, pie[w_k].y, pie[i].x, pie[i].y):
                # print("rook = ",pie[i].x,pie[i].y)
                val = True
        
        #check with black bishop
        if pie[i].rank == 'b' and pie[i].family == "black":
            if diagonalcheck(pie[w_k].x, pie[w_k].y, pie[i].x, pie[i].y):
                if diagonal(pie[w_k].x, pie[w_k].y, pie[i].x, pie[i].y):
                    val = True

        #check with black queen
        if pie[i].rank == 'q' and pie[i].family == "black":
            if check_rook(pie[w_k].x, pie[w_k].y, pie[i].x, pie[i].y):
                val = True
            if diagonalcheck(pie[w_k].x, pie[w_k].y, pie[i].x, pie[i].y):
                if diagonal(pie[w_k].x, pie[w_k].y, pie[i].x, pie[i].y):
        ...
        #check with black knight
        if pie[i].rank == 'k' and pie[i].family == "black":
            if knight_check(pie[w_k].x, pie[w_k].y, pie[i].x, pie[i].y):
                val = True

    # check for black king
    for i in range(len(pie)):
        #check with black pawn
        if pie[i].rank == 'p' and pie[i].family == "white":
            if pie[i].y == pie[b_k].y - 1 :
                if pie[i].x == pie[b_k].x+1 or pie[i].x == pie[b_k].x-1:
                    val = True

        #check with white rook
        if pie[i].rank == 'r' and pie[i].family == "white":
            if check_rook(pie[b_k].x, pie[b_k].y, pie[i].x, pie[i].y):
                # print("rook = ",pie[i].x,pie[i].y)
                val = True
        
        #check with white bishop
        if pie[i].rank == 'b' and pie[i].family == "white":
            if diagonalcheck(pie[b_k].x, pie[b_k].y, pie[i].x, pie[i].y):
                if diagonal(pie[b_k].x, pie[b_k].y, pie[i].x, pie[i].y):
                    val = True

        #check with white queen
        if pie[i].rank == 'q' and pie[i].family == "white":
            if check_rook(pie[b_k].x, pie[b_k].y, pie[i].x, pie[i].y):
                val = True
            if diagonalcheck(pie[b_k].x, pie[b_k].y, pie[i].x, pie[i].y):
                if diagonal(pie[b_k].x, pie[b_k].y, pie[i].x, pie[i].y):
        ...
        #check with white knight
        if pie[i].rank == 'k' and pie[i].family == "white":
            if knight_check(pie[b_k].x, pie[b_k].y, pie[i].x, pie[i].y):
                val = True

    return val

def strarting_page():
    print("inside starting page")
    # selec = True
    myfont = pygame.font.Font("freesansbold.ttf", 40)
    string1 = "Multiplayer Chess Game"
    string2 = "Developed By:"
    string3 = "Kapil & Gaurav"
    pygame.draw.rect(game_display, d_white, (0, 0, 600, 680))
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            quit()
    label1 = myfont.render(string1, 1, black)
    label2 = myfont.render(string2, 1, black)
    label3 = myfont.render(string3, 1, black)
    game_display.blit(label1, (68, 100))
    game_display.blit(label2, (170, 260))
    game_display.blit(label3, (160, 320))
    pygame.display.update()
    time.sleep(2)
...
def game():
    ...
    while True:
        val = check_king_life()
        danger = check_king_danger()
        if danger:
            myfont = pygame.font.Font("sans.ttf", 30)
            string = "Check"
            label = myfont.render(string, 1, red)
            game_display.blit(label, (250, 620))
            pygame.display.update()
            time.sleep(1)
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if not selec:
                    ...
                    selec = False
            if event.type == pygame.QUIT or not val:
                pygame.quit()
                quit()
        board_draw()
